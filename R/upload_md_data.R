...
upload_md_data <- function(md_out_events){
  req_names <- c("deployment", "file", "conf", "category", "x_off", "y_off", "width", "height", "bbox_category","image_category",  "datetimeoriginal")
  ...
  if(RPostgreSQL::dbExistsTable(con,  c("import", "megadetector"))){
    ...
    megadetector_classifications_max <- RPostgreSQL::dbGetQuery(con, glue::glue_sql("SELECT max(obs_id) as max FROM import.megadetector", directory_ids = unique(new_classifications$directory_id), .con = con))$max
  ...
  if(nrow(new_classifications) > 0){
    new_classifications <-
    new_classifications %>%
      dplyr::mutate(obs_id = paste0(deployment, "_", seq(dplyr::n()) + megadetector_classifications_max), .before = "deployment")
